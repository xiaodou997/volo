...
### Changed
- macOS workspace 路径原生化：应用图标改由 `NSWorkspace.iconForFile` + `NSImage` 原生 64×64 重采样 + `NSBitmapImageRep` 生成 PNG data URI，Finder reveal 改用 `activateFileViewerSelectingURLs`；移除 `sips` / `open -R` helper subprocess，并新增 macOS 26 workspace smoke 与防回退 guard
